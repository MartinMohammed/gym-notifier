...
class Studio: 
    ...
    def instantiate_studios(cls) -> None: 
        ...
        The method first retrieves all the studios and their data from the website by calling the __get_all_occupancy_data() method of the class.

        Then, the for loop iterates over each studio in the studios list, and extracts the following information for each studio:
            - Row number
            - Location
            - Current number of people
            - Maximum number of people allowed

        The method creates a new Studio object with the extracted data and adds it to the all_dict dictionary with the location as the key:
        ...
        Overall, this method initializes the Studio objects and makes them available in the all_dict dictionary for other methods to use.
        """
    ...
    @classmethod
    def notify_on_people_amount_criteria(cls, time_interested_in: dict[str, int], minimum: int = None, maximum: int = None, specific_studio_name = "", telegram_chat_id: str = "", recipient_name: str = ""):
        '''
        Parameters:
    ...
    def __get_occupancy_data(studio_name: str, cb= None):
        '''
        :param studios_name: A string representing the name of the studio whose data is to be updated.
        :param cb: An optional callback function that receives the fetched Studio data.
        This is a static method called __get_occupancy_data in the Studio class. It updates the data of a specified studio.
        ...
        - If the studio name is valid, the method proceeds to fetch the data for the specified studio from a URL using the make_http_request function.
        - The fetched data is then parsed using the BeautifulSoup library, and the relevant data is extracted from the parsed HTML. 
    ...
    def __extract_studio_data_from_row(studio_row) -> dict:
        ...
        This function extracts the location, current occupancy, and maximum occupancy data from a list of table data cells for a single studio, performs some validation, and returns a dictionary containing the extracted data.
        The function first extracts the location data by stripping the leading and trailing whitespaces and raises an assertion error if the location string is empty.
        It then tries to convert the maximum and current occupancy data from string to integers using the int() function and catches a ValueError if the conversion fails. In case of an error, the function logs the error message using the root_logger object and includes the exception traceback information.
